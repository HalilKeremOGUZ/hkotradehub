...
export default async function Home({params}:{params:Promise<{locale:string}>}){
 const {locale}=await params;if(!isLocale(locale))notFound();const d=dictionaries[locale];const industries=industryCopy[locale];const services=serviceCopy[locale];const process=steps[locale];
 ...
 return <>
  <section className="noise hero-grid relative overflow-hidden bg-hero-glow">
   ...
   <div className="container-shell grid min-h-[760px] items-center gap-14 py-20 lg:grid-cols-[1.04fr_.96fr] lg:py-24">
    ...
    <Reveal className="relative">
     <div className="absolute -left-7 top-16 z-10 hidden rounded-2xl border border-white/50 bg-white/85 p-4 shadow-soft backdrop-blur-xl sm:block"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-aqua/20 text-ocean"><BadgeCheck size={20}/></span><div><b className="block text-sm">Verified approach</b><small className="text-ink/45">Trust before transaction</small></div></div></div>
     <div className="relative ml-auto min-h-[570px] max-w-[600px] overflow-hidden rounded-[2.5rem] bg-ink shadow-[0_38px_100px_rgba(8,20,39,.22)]">
      <Image src="https://images.unsplash.com/photo-1494412574643-ff11b0a5c1c3?auto=format&fit=crop&w=1600&q=88" alt="International container shipping and global trade" fill priority sizes="(max-width:1024px) 100vw, 50vw" className="object-cover"/>
      <div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/15 to-transparent"/>
      <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8"><p className="text-[10px] font-bold uppercase tracking-[.24em] text-aqua">Specialized corridor</p><div className="mt-3 flex items-end justify-between gap-6"><div><h2 className="text-2xl font-semibold text-white">Türkiye ⇄ Chile</h2><p className="mt-2 max-w-sm text-sm leading-6 text-white/58">Local insight. Global standards. Practical trade execution.</p></div><span className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-white/20 bg-white/10 text-white backdrop-blur"><Ship size={21}/></span></div></div>
     </div>
     <div className="absolute -bottom-5 -right-4 z-10 rounded-2xl border border-white/50 bg-white/90 p-4 shadow-soft backdrop-blur-xl sm:right-8"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-copper/15 text-copper"><Globe2 size={20}/></span><div><b className="block text-sm">Global network</b><small className="text-ink/45">Built to expand beyond two markets</small></div></div></div>
    </Reveal>
   </div>
  </section>
  ...
  <section className="section bg-mist"><div className="container-shell"><div className="grid overflow-hidden rounded-[2.5rem] bg-ink lg:grid-cols-2"><Reveal className="p-8 text-white sm:p-12 lg:p-16"><p className="eyebrow !text-aqua">{d.home.trustEyebrow}</p><h2 className="mt-5 text-3xl font-semibold leading-tight tracking-[-.035em] sm:text-4xl">{d.home.trustTitle}</h2><p className="mt-5 max-w-xl leading-8 text-white/58">{d.home.trustText}</p><div className="mt-9 grid gap-4 sm:grid-cols-2"><Trust icon={BadgeCheck} title="Business verification"/><Trust icon={ShieldCheck} title="Risk-aware process"/><Trust icon={PackageSearch} title="Commercial due diligence"/><Trust icon={CircleDollarSign} title="Practical deal support"/></div></Reveal><div className="relative min-h-[430px]"><Image src="https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=1400&q=86" alt="International business team reviewing a trade opportunity" fill sizes="(max-width:1024px) 100vw, 50vw" className="object-cover"/><div className="absolute inset-0 bg-gradient-to-r from-ink/70 via-transparent to-transparent lg:bg-gradient-to-r"/></div></div></div></section>
