...
export default async function Page({params}:{params:Promise<{locale:string}>}){
  ...
  return <><PageHero eyebrow={t.eyebrow} title={t.title} text={t.text}/><section className="section"><div className="container-shell grid gap-7 md:grid-cols-2 lg:grid-cols-3">{regularProducts.map(p=><ProductCard product={p} locale={locale} key={p.slug}/>)}<SparePartsHub locale={locale}/><Link href={`/${locale}/quote`} className="group overflow-hidden rounded-[1.75rem] border border-ink/8 bg-white p-6 shadow-[0_20px_60px_rgba(8,20,39,.06)] transition hover:-translate-y-1.5 hover:shadow-[0_28px_80px_rgba(8,20,39,.13)]"><div className="relative h-60 overflow-hidden rounded-2xl"><Image src="/custom-request.png" alt={t.customTitle} fill className="object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-ink/65 via-transparent to-transparent"/><span className="absolute bottom-4 left-4 grid h-10 w-10 place-items-center rounded-full bg-white/15 text-white backdrop-blur"><Plus size={22} strokeWidth={1.5}/></span></div><h3 className="mt-6 text-xl font-semibold tracking-tight">{t.customTitle}</h3><p className="mt-3 text-sm leading-6 text-ink/58">{t.customText}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-ocean">{t.customCta}<ArrowUpRight size={16}/></span></Link></div></section></>;
}
